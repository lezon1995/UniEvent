...
namespace UniEvent.Editor
{
    internal static class DiagnosticsExtensions
    {
        ...
        static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            if (declareType.StartsWith("UniEvent"))
            {
                return true;
            }
        ...
        static string AppendHyperLink(string path, string line)
        {
            ...
            var fileName = fi.FullName.Replace(Path.DirectorySeparatorChar, '/').Replace(Application.dataPath, "");
            if (Path.GetPathRoot(fileName).Contains(":"))
            {
                return fileName + ":" + line; // without link outside of project dir
            }

            string withAssetsPath;
            if (fileName.StartsWith("/"))
            {
                withAssetsPath = "Assets" + fileName;
            }
            else
            {
                withAssetsPath = "Assets/" + fileName;
